#9 - KLD (chairs retained). Novelty, transience, and resonance calculated across the whole corpus, chairs kept as centre speeches - face-validity check per Barron SI section 2.4
import numpy as np
import pandas as pd
import os

scales = [1,60,250,1000,7500] #must match the main NTR script
...
output_csv = os.path.join(source, "hansard_ntr_by_scale_chairs.csv") #separate path, keeps the live results untouched
...
#=========chairs kept as centre speeches, per Barron SI 2.4 face-validity check ======================================
corpus["original_analysis_order"] = corpus["analysis_order"] #joins back to the corpus
corpus["kld_row"] = np.arange(len(corpus)) #position in the full sequence, chairs included
